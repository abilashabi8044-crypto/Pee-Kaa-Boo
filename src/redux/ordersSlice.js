  ...
  reducers: {
    addOrders: (state, action) => {
      ...
      if (orderObj && orderObj.orderId !== '973675159' && orderObj.id !== '973675159') {
        ...
        if (!alreadyExists) {
          state.items.unshift(orderObj);
        }
        state.latestOrder = orderObj;
        try {
          localStorage.setItem('pkb_orders', JSON.stringify(state.items));
          localStorage.setItem('pkb_latest_order', JSON.stringify(orderObj));
        } catch (e) {
          console.error(e);
        }
      }
    ...
    removeOrder: (state, action) => {
      ...
      state.items = state.items.filter(order => order.id !== targetId && order.orderId !== targetId);
      if (state.latestOrder && (state.latestOrder.orderId === targetId || state.latestOrder.id === targetId)) {
        state.latestOrder = state.items[0] || null;
      }
      try {
        localStorage.setItem('pkb_orders', JSON.stringify(state.items));
        if (state.latestOrder) {
          localStorage.setItem('pkb_latest_order', JSON.stringify(state.latestOrder));
        } else {
          localStorage.removeItem('pkb_latest_order');
        }
      } catch (e) {
    ...
    cancelOrder: (state, action) => {
      ...
      if (state.latestOrder && (state.latestOrder.orderId === targetId || state.latestOrder.id === targetId)) {
        state.latestOrder = { ...state.latestOrder, status: 'cancelled' };
      }
...
export const { addOrders, cancelOrder, removeOrder } = ordersSlice.actions;
